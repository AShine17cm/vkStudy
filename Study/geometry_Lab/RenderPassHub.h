...
struct RenderPassHub
{
	...
	void init(VulkanDevice* vulkanDevice,VkSurfaceKHR surface, uint32_t WIDTH,uint32_t HEIGHT, VkFormat depthFormat) 
	{
	...
	void createFrameBuffers(Resource* res) 
	{
		...
		framebuffers::createFramebuffers(shadowExtent, shadowPass, shadowViews, 1,device, &shadow_FB);
	}
	void clean() 
	{
