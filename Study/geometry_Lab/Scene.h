...
struct  Scene
{
	...
	/*
	用idx 将 geo 以不同的方式渲染
	为了用同一个流程渲染 Geometry 进行调试，PushConstants 使用了 VK_SHADER_STAGE_GEOMETRY_BIT
	*/
	void draw(VkCommandBuffer cmd, VkPipelineLayout piLayout, int batchIdx)
	{
		...
		case 0:
		{
			...
			for (uint32_t i = 0; i < pillars.size(); i++)
			{
				...
				vkCmdPushConstants(cmd, piLayout, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_GEOMETRY_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0, size, &pod);
