...
private:
    ...
    void recordCommandBuffer(VkCommandBuffer cmd, uint32_t imageIndex) 
    {
        ...
        if (true) 
        {
            ...
            drawScene(cmd, frame);
            /* 调试法线 */
            if (input->displayNormals) 
            { 
                debugGeoNormal(cmd, frame);
            }
            vkCmdEndRenderPass(cmd);
        }
    ...
    void debugGeoNormal(VkCommandBuffer cmd, Frame* frame) 
    {
        uint32_t batchIdx = -1;
        ...
        for (uint32_t i = 0; i < 4; i++)
        {
            if (i == 2)continue;
            batchIdx = i;
            scene.draw(cmd, piHub.piLayout_geo_ubo, batchIdx);
        }
        /* 实例化的geo-debug*/
        vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, piHub.pi_geo_debug_instancing);
        vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, piHub.piLayout_geo_ubo_instancing, 0, 1, &frame->geo_ubo_ubo, 0, nullptr);
        batchIdx = 2;
        scene.draw(cmd, piHub.piLayout_geo_ubo_instancing, batchIdx);
    }
