...
struct Frame
{
	...
	VkDescriptorSet setInstance;		//tex				instance
	...
	void prepare(VulkanDevice* vulkanDevice, VkDescriptorPool descriptorPool, PipelineHub* pipes)
	{
		...
		descriptors::allocateDescriptorSet(&pipes->setLayout_Instance, 1, descriptorPool, device, &setInstance);
	...
	void updateDescritors(VkDevice device, Resource* res)
	{
		...
		mg::descriptors::writeDescriptorSet(types.data(), infos.data(), counts.data(), counts.size(), setInstance, device);
	}
