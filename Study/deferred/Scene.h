...
struct  Scene
{
	...
	void update(Frame* pFrame, float time, float deltaTime)
	{
		if (input->flipShadows) //3个光源的阴影,逐次展示，共同展示
		{
			...
			shadowFliper = (shadowFliper + 1) % 4;
			glm::ivec4 debugShadow = { 0,0,0,0 };
			switch (shadowFliper)
			{
			case 0:
				debugShadow = { 1,0,0,0 };
			...
			ui.debugShadow = debugShadow;
			view->data.debugShadow = debugShadow;
		}
		/* 调试延迟渲染 */
		if (input->flip_Deferred) {
			...
			{
			case 0:
				debugDeferred = { 1,0,0,0 };
				break;
			case 1:
				debugDeferred = { 0,1,0,0 };
				break;
			case 2:
				debugDeferred = { 0,0,1,0 };
				break;
			case 3:
