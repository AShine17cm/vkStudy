...
struct Frame
{
	...
	VkDescriptorSet setInstanceShade;	//tex				instance
	...
	void prepare(VulkanDevice* vulkanDevice, VkDescriptorPool descriptorPool, PipelineHub* pipes)
	{
		...
		descriptors::allocateDescriptorSet(&pipes->setLayout_InstanceShade, 1, descriptorPool, device, &setInstanceShade);
	...
	void updateDescritors(VkDevice device, Resource* res)
	{
		...
		counts = { 1};
		infos = {&res->tex_Instance->descriptor };
		mg::descriptors::writeDescriptorSet(types.data(), infos.data(), counts.data(), counts.size(), setInstanceShade, device);
	}
