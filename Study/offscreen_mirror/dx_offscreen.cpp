...
private:
    ...
    void initVulkan() {
        ...
        createRenderPass();
        /* Offscreen */
        offscr = new OffscreenPass();
        offscr->extent = { 512,512 };
        offscr->prepare(vulkanDevice,swapchain->imageFormat,depthFormat,renderPass);
    ...
    void cleanup() {
        ...
        scene.cleanup();
        /* Offscreen */
        offscr->cleanup(vulkanDevice->logicalDevice);
    ...
    void createResources() 
    {
        ...
        for (uint32_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) 
        {
            ...
            frames[i].updateDescritors(device,&resource,offscr);
        }
    }
    ...
    void recordCommandBuffer(VkCommandBuffer cmd, uint32_t imageIndex) 
    {
        ...
        uint32_t dstSet = 0;    //set=0,binding=0       场景矩阵 view-proj  多个shader公用
        /* Offscreen */
        mg::batches::BeginRenderpass(cmd,renderPass, offscr->frameBuffer, clears.data(), 2, offscr->extent);
        mg::batches::SetViewport(cmd, offscr->extent);
        vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, piHub.piLayout_scene, dstSet, 1, &frame->setScene_0, 0, nullptr);
        
        //此时offscr-color:   VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL
        //此时offscr-depth:   VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL
        drawScene(cmd, frame,false);     
        vkCmdEndRenderPass(cmd);
        //此时offscr-color:  VK_IMAGE_LAYOUT_PRESENT_SRC_KHR
        //此时offscr-depth:  和 final_layout一致(shader_read_only)

        VkImageSubresourceRange range = { VK_IMAGE_ASPECT_COLOR_BIT,0,1,0,1 };
        textures::setImageLayout(cmd, offscr->color->image, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, &range);
        //range = { VK_IMAGE_ASPECT_DEPTH_BIT|VK_IMAGE_ASPECT_STENCIL_BIT,0,1,0,1 };
        //textures::setImageLayout(cmd, offscr->depth->image, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL, VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL, &range);
        /* 正常渲染 */
    ...
    void drawScene(VkCommandBuffer cmd, Frame* frame,bool mirrorPlane) 
    {
        uint32_t dstSet = 0;
        /* pipeline-A */
        vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, piHub.pi_A);
        dstSet = 1;             //set=1,binding=0,1,2   data-array-tint-texture
        vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, piHub.piLayout_A, dstSet, 1, &frame->setA_1, 0, nullptr);
        uint32_t batchIdx = 0;
        scene.draw(cmd, piHub.piLayout_A, batchIdx);

        /* pipeline-B */
        vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, piHub.pi_B);
        dstSet = 1;             //set=1,binding=0,1     tint-texture
        vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, piHub.piLayout_B, dstSet, 1, &frame->setB_1, 0, nullptr);
        batchIdx = 1;
        scene.draw(cmd, piHub.piLayout_B, batchIdx);

        if (!mirrorPlane)return;
        /* 镜像的角度 */
        /* 用于测试Material的概念 同样的Layout 不同的descriptor-set */
        vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, piHub.piLayout_B, dstSet, 1, &frame->setC_1, 0, nullptr);
        batchIdx = 2;
        scene.draw(cmd, piHub.piLayout_B, batchIdx);
    }
