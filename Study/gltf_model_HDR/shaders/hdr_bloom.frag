#version 450

layout (set=0, binding = 0) uniform sampler2D samplerColor0;
layout (set=0, binding = 1) uniform sampler2D samplerColor1;

layout (location = 0) in vec2 inUV;

layout (location = 0) out vec4 outColor;

//layout (constant_id = 0) const int dir = 0;
layout (push_constant) uniform Pushs
{
	vec4 dataA;
	ivec4 dataB;
}pushs;

void main(void)
{
	// From the OpenGL Super bible
	const float weights[] = float[](0.0024499299678342,
									0.0043538453346397,
									0.0073599963704157,
									0.0118349786570722,
									0.0181026699707781,
									0.0263392293891488,
									0.0364543006660986,
									0.0479932050577658,
									0.0601029809166942,
									0.0715974486241365,
									0.0811305381519717,
									0.0874493212267511,
									0.0896631113333857,
									0.0874493212267511,
									0.0811305381519717,
									0.0715974486241365,
									0.0601029809166942,
									0.0479932050577658,
									0.0364543006660986,
									0.0263392293891488,
									0.0181026699707781,
									0.0118349786570722,
									0.0073599963704157,
									0.0043538453346397,
									0.0024499299678342);


	float blurScale=pushs.dataA.x;
	float blurStrength=pushs.dataA.y;
	float ar = 1.0;
	//Œ∆¿Ì¥Û–°
	//vec2 ts = textureSize(samplerColor1, 0);


	int n=weights.length();
	vec2 P=inUV-vec2(n/2,n/2)*blurScale;

	vec4 color = vec4(0.0);
	for (int u = 0; u <n; u++)
	{
		for(int v=0;v<n;v++)
		{
			vec2 uv=P+vec2(u,v)*blurScale;
			color += texture(samplerColor1, uv) * weights[u]*weights[v] * blurStrength;
		}
	}
	outColor = color;
	//outColor=texture(samplerColor1,inUV);
}