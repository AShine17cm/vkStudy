...
/*	生成 brdf查找贴图 lut
	生成辉度图 irradiace
	生成预过滤贴图 prefilter-map
  */
PbrEnv::PbrEnv(mg::VulkanDevice* vulkanDevice,const char* emptyFile,const char* envFile,const char* skyboxFile)
{
	this->vulkanDevice = vulkanDevice;
	this->device = vulkanDevice->logicalDevice;

	empty.loadFromFile(emptyFile, VK_FORMAT_R8G8B8A8_UNORM, vulkanDevice, vulkanDevice->graphicsQueue);
	environmentCube.loadFromFile(envFile, VK_FORMAT_R16G16B16A16_SFLOAT, vulkanDevice, vulkanDevice->graphicsQueue);
	skybox.loadFromFile(skyboxFile, vulkanDevice, vulkanDevice->graphicsQueue);
...
void PbrEnv::generateCubemaps()
{
	...
	for (uint32_t target = 0; target < PREFILTEREDENV + 1; target++) {
		...
		switch (target) {
		case IRRADIANCE:
			this->irradianceCube = cubemap;
			break;
		case PREFILTEREDENV:
			this->prefilteredCube = cubemap;
			this->prefilteredCubeMipLevels = static_cast<float>(numMips);
			break;
		};
...
VkPipelineShaderStageCreateInfo PbrEnv::loadShader(VkDevice device, std::string filename, VkShaderStageFlagBits stage)
{
