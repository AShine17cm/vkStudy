...
namespace vkglTF
{
	...
	struct Texture {
		...
		void fromglTfImage(tinygltf::Image& gltfimage, TextureSampler textureSampler, mg::VulkanDevice* device, VkQueue copyQueue);
	};
	...
	struct Mesh {
		...
		struct UniformBlock 
		{
			glm::mat4 matrix;	//到根节点的矩阵
			//glm::mat4 jointMatrix[MAX_NUM_JOINTS]{};
			//float jointcount{ 0 };
		} uniformBlock;
		Mesh(mg::VulkanDevice* device, glm::mat4 matrix);
		~Mesh();
		void setBoundingBox(glm::vec3 min, glm::vec3 max);
	};
	...
	struct Model {
		...
		void loadTextures(tinygltf::Model& gltfModel, mg::VulkanDevice* device, VkQueue transferQueue);
		VkSamplerAddressMode getVkWrapMode(int32_t wrapMode);
		VkFilter getVkFilterMode(int32_t filterMode);
		void loadTextureSamplers(tinygltf::Model& gltfModel);
		void loadMaterials(tinygltf::Model& gltfModel);
		void loadFromFile(std::string filename, mg::VulkanDevice* device, VkQueue transferQueue, float scale = 1.0f);
