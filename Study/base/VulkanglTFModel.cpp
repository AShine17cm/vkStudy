...
namespace vkglTF
{
	...
	void Texture::fromglTfImage(tinygltf::Image &gltfimage, TextureSampler textureSampler, mg::VulkanDevice *device, VkQueue copyQueue)
	{
		this->device = device;
		...
		MG_CHECK_RESULT(vkCreateBuffer(device->logicalDevice, &bufferCreateInfo, nullptr, &stagingBuffer));
		vkGetBufferMemoryRequirements(device->logicalDevice, stagingBuffer, &memReqs);
		memAllocInfo.allocationSize = memReqs.size;
		memAllocInfo.memoryTypeIndex = device->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
		MG_CHECK_RESULT(vkAllocateMemory(device->logicalDevice, &memAllocInfo, nullptr, &stagingMemory));
		MG_CHECK_RESULT(vkBindBufferMemory(device->logicalDevice, stagingBuffer, stagingMemory, 0));

		uint8_t *data;
		MG_CHECK_RESULT(vkMapMemory(device->logicalDevice, stagingMemory, 0, memReqs.size, 0, (void **)&data));
		...
		MG_CHECK_RESULT(vkCreateImage(device->logicalDevice, &imageCreateInfo, nullptr, &image));
		vkGetImageMemoryRequirements(device->logicalDevice, image, &memReqs);
		memAllocInfo.allocationSize = memReqs.size;
		memAllocInfo.memoryTypeIndex = device->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
		MG_CHECK_RESULT(vkAllocateMemory(device->logicalDevice, &memAllocInfo, nullptr, &deviceMemory));
		MG_CHECK_RESULT(vkBindImageMemory(device->logicalDevice, image, deviceMemory, 0));
		...
		MG_CHECK_RESULT(vkCreateImageView(device->logicalDevice, &viewInfo, nullptr, &view));
	...
	// Mesh
	Mesh::Mesh(mg::VulkanDevice *device, glm::mat4 matrix) {
		this->device = device;
		...
		MG_CHECK_RESULT(device->createBuffer(
			...
			&uniformBlock));
		MG_CHECK_RESULT(vkMapMemory(device->logicalDevice, uniformBuffer.memory, 0, sizeof(uniformBlock), 0, &uniformBuffer.mapped));
	...
	void Model::loadTextures(tinygltf::Model &gltfModel, mg::VulkanDevice *device, VkQueue transferQueue)
	{
	...
	void Model::loadFromFile(std::string filename, mg::VulkanDevice* device, VkQueue transferQueue, float scale)
	{
